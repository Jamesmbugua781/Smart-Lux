...
export function LocationCard({ location, isSelected = false, onViewDetails }: LocationCardProps) {
  return (
    <div className={`flex flex-col gap-4 rounded-2xl border bg-white p-4 shadow-[0_8px_25px_rgba(11,31,58,0.03)] transition hover:-translate-y-0.5 hover:shadow-[0_10px_35px_rgba(11,31,58,0.06)] sm:flex-row sm:items-center sm:justify-between ${isSelected ? 'border-[var(--color-primary)]' : 'border-[#edf0f2]'}`}>
      ...
      <button
        ...
        className="inline-flex shrink-0 items-center justify-center gap-2 rounded-full border border-[#dfe4e8] bg-[#f8f8f5] px-3 py-1.5 text-sm font-medium text-[#0B1F3A] transition hover:border-[var(--color-primary-soft-strong)] hover:bg-[var(--color-primary-soft)]"
      >
